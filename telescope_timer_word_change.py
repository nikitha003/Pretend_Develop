...
#camel to underscore --> done
#pickle db unavailability --> solved
#key unavailability --> solved
#deleting redundant variables --> done
#debug level change --> done
#collision detection switching ON --> done
...
def destroy_boss_screen():
    boss_label.destroy()

def boss_key(event):
    print("Inside boss_key")
    global boss_label
    boss_img = PhotoImage(file = "Milky_Way_Arch.png")
    boss_label = Label(sky_canvas, text = "balance_sheet.py", font = ("Arial", 20), bg = "black", fg = "white", image = boss_img)
    boss_label.place(x = 0, y = 0)
    sky_canvas.after(20000, destroy_boss_screen)
