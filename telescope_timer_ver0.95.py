...
def pallasAppear():
    ...
    print("Inside pallasAppear Player name entry: " + player_name_entry)
    #score = score_label.cget("text") #player_dict[player_name_entry]
    print("Inside pallasAppear Score  " + str(score))
    #if score >= 0:
    ...
    if score > 200:
        ...
        pallasY = pallasY + 40 #Adding above 30
...
def collision_detection_pallas():
    ...
    if gameStart:
        ...
        global gameOver
        telescopeBB = skyCanvas.bbox(telescopeImage)
        pallasBB = skyCanvas.bbox(pallasImage)
        #print(telescopeBB[0])
        #print(vestaBB[2])
        if telescopeBB[0] < pallasBB[2] < telescopeBB[2] and telescopeBB[1] < pallasBB[3] < telescopeBB[3]:
            skyCanvas.delete(telescopeImage)
            stop()
            sys.exit()
...
def start_screen_appear():
    global start_label
    start_label = Label(skyCanvas, text = "Supernova Hunter\n You have to move the space telescope to capture the Supernova\nThe Game will become progressively difficult as the score goes high\nGame will become faster and more asteroids will come", font = ("Arial", 20), bg = "black", fg = "white")
    start_label.place(x = 500, y = 500)
    skyCanvas.after(10000, destroy_start_screen)
...
#Leader Board Label
leader_label = Label(skyCanvas, text = "Leader Board", font = ("Arial", 15), bg = "black", fg = "white")
...
skyCanvas.after(1000, start_screen_appear)
